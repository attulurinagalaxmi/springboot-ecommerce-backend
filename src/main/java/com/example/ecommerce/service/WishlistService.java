...
public class WishlistService {
	 ...
	 public void addProductToWishlist(Long productId) {
	        ...
	        Product product = productRepository.findById(productId)
	                .orElseThrow(() -> new UserNotFoundException("Product not found: " + productId));
	 ...
	 public void removeProductFromWishlist(Long productId) {
		 Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
			...
			User user = userRepository.findByEmail(email).orElseThrow(() -> new UserNotFoundException("User not found"));
	        Wishlist wishlist = wishlistRepository.findByUserId(user.getId())
	                .orElseThrow(() -> new UserNotFoundException("Wishlist not found for user: " + user.getId()));
	    ...
	    public WishlistResponseDTO getUserWishlistProducts() {
	        ...
	        Wishlist wishlist = wishlistRepository.findByUserIdWithProducts(user.getId())
	                .orElseThrow(() -> new UserNotFoundException("Wishlist empty or not found for user: " + user.getId()));
